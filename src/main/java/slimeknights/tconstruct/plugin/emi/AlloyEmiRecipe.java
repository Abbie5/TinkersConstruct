...
public class AlloyEmiRecipe implements EmiRecipe {
  ...
  public EmiRecipeCategory getCategory() {
    return EMIPlugin.ALLOY_CATEGORY;
  }
  ...
  public void addWidgets(WidgetHolder widgets) {
    ...
    // output
    widgets.addSlot(getOutputs().get(0).copy().setAmount(maxAmount), 137, 11)
      .customBackground(null, 0, 0, 16, 32)
      .drawBack(false)
      .recipeContext(this);

    // fuel
    EmiIngredient fuel = EmiIngredient.of(MeltingFuelHandler.getUsableFuels(recipe.getTemperature()).stream()
      .map(f -> FluidEmiStack.of(f.getFluid(), 1L)).toList());
